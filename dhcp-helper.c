...
int main(int argc, char **argv)
{
  ...
  while (1)
	...
	case 's': case 'b': case 'i': case 'e':
	  {
	    struct namelist *new = malloc(sizeof(struct namelist)); //выделяем память для новых данных
	    
	    if (!new)  //Если вернулся нулевой укзатеь
	      {
  ...
  if (setsockopt(fd, SOL_IP, IP_PKTINFO, &oneopt, sizeof(oneopt)) == -1 ||   //Возвращение дополнительной информации об интерфейсе
      setsockopt(fd, SOL_SOCKET, SO_BROADCAST, &oneopt, sizeof(oneopt)) == -1 ||  //Получение броадкастовых сообщений
      setsockopt(fd, SOL_IP, IP_MTU_DISCOVER, &mtuopt, sizeof(mtuopt)) == -1)  //Убирает флаг "не фрагментировать"
    {
...
if (!debug)
  ...
  while (1) {
    ...
    while (1) { // Прокручивать вечность =)
      size_t newsz;
      struct dhcp_packet *newbuf;

      msg.msg_flags = 0;// Учиановили флпги в 0 
      while((sz = recvmsg(fd, &msg, MSG_PEEK)) == -1 && errno == EINTR); //выход будет, когда мы примем сообщение без ошибки, и с ошибкой не равной EINTR
      
      if (sz == -1 || !(msg.msg_flags & MSG_TRUNC) ||  //если сообщение было принято полностью
	  !(newbuf = realloc(packet, (newsz = buf_size + 100)))) //увеличть размер текущего буфера для пакета
	break;
    ...
    while ((sz = recvmsg(fd, &msg, 0)) == -1 && errno == EINTR); //выход будет, когда мы примем сообщение без ошибки, или с ошибкой не равной EINTR
    
   // if ((msg.msg_flags & MSG_TRUNC) || //если флаг поднят MSG_TRUNC или
//	sz < (ssize_t)(sizeof(struct dhcp_packet)) || // если байт ринято меньше чем длина структуры dhcp_packet
//	msg.msg_controllen < sizeof(struct cmsghdr))   //или контрольная длина сообщения меньше, чем длина структуры cmsghdr 
 // если с принятым пакетом что-то не так.
  //    continue; // больше в этом цикле действия никакие не выполняются.
    
    iface_index = 0;
    for (cmptr = CMSG_FIRSTHDR(&msg); cmptr; cmptr = CMSG_NXTHDR(&msg, cmptr)) //Тест проводится до тех пор пока cmptr!=NULL, аолучаем указатели на структуру msg 
