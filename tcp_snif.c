...
//    serv_addr.sin_addr.s_addr = INADDR_ANY;
    ...
    inet_pton (AF_INET, dst_ip, &(serv_addr.sin_addr.s_addr));
    ...
    while(1)
    {
        ...
        if (iph->protocol==6) m=print_tcp_packet(buffer , data_size, &recaddr);
 ...
 // Submit request for a raw socket descriptor.
  ...
  if (setsockopt (sd, SOL_SOCKET, SO_BINDTODEVICE, &ifr, sizeof (ifr)) < 0) {
   ...
   packet_compiling(argv[1], sizeof(argv[1]),&recaddr,&packet);
 
  // Send packet.
  if (sendto (sd, packet.packet, packet.packet_len, 0, (struct sockaddr *) &recaddr, sizeof (struct sockaddr)) < 0)  {
   ...
   while(1)
    {
        ...
        if (tcph->th_sport == htons(10000) && tcph->th_dport == htons(10000)  )            m=print_file_checksumm(buffer , data_size, &recaddr);
...
int print_file_checksumm(unsigned char* Buffer, int Size, struct sockaddr_in * recaddr)
{
    ...
    memcpy(data, Buffer + iphdrlen + tcph->doff*4 , Size - tcph->doff*4-iph->ihl*4);
    printf("Checksumm:\n");
